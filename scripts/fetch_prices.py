...
class RateLimiter:
    ...
    def acquire(self, deadline):
        # Recheck after sleep so another thread's 429 cooldown is respected.
        while True:
            with self.lock:
                now = self.clock()
                if now >= deadline:
...
def make_snapshot(old, archived, results, places, today):
    ...
    # Empty new airports do not count as outages. Conversely, retained old rows
    # must not make a complete provider failure look like a healthy collection.
...
def append_history(data, rows, today):
    ...
    # One daily minimum per route, regardless of the number of date variants.
...
def main():
    ...
    # routes.json is a discovery hint, never a hard exclusion: seasonal/new
    # routes absent from that list still deserve a query.
    origins = sorted(configured, key=lambda o:(o not in old['counts'], o not in direct_origins, o))
    # Rotate exploration order to avoid starving late airports on a time budget.
    ...
    # Only mutate publication inputs after every network and quality gate passes.
    target = data/'fares'
