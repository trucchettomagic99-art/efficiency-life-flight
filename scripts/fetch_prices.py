...
try:
    from location_authority import is_commercial_airport
except ImportError:
    from scripts.location_authority import is_commercial_airport
...
def write_airport_module(catalog):
    ...
    codes = sorted({a.get('i') for a in catalog.get('airports', [])
                    if isinstance(a.get('i'), str) and len(a['i']) == 3 and is_commercial_airport(a['i'])})
